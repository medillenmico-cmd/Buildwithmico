...
export default function SplitFlapText({
  ...
  const sourceWords = words.length > 0 ? words : DEFAULT_WORDS;
  const phrasesKey =
    typeof text === 'string'
      ? text
  ...
  const width = useMemo(() => {
    const longest = phrases.reduce(
      (maximum, phrase) => Math.max(maximum, phrase.length),
      1,
    );
  ...
  useEffect(() => {
    ...
    const safeCycleDelay = Math.max(240, Number(cycleDelay) || 2400);
    ...
    const notifyComplete = () => {
      if (
        !loop &&
        phraseIndex === normalizedPhrases.length - 1 &&
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const totalDuration = plans.reduce(
        (maximum, plan) =>
          Math.max(maximum, plan.start + plan.sequence.length * safeFlipMs),
        0,
      );
          ...
          if (step < plan.sequence.length) {
            ...
            if (step !== plan.step) {
              ...
              updates.push({
                ...
                current:
                  step === 0
                    ? fromPhrase[plan.index] || ' '
                    : plan.sequence[step - 1],
